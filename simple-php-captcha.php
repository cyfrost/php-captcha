...
function simple_php_captcha($id = "", $config = array()) {
    ...
    if ( strpos($_SERVER['SCRIPT_FILENAME'], $_SERVER['DOCUMENT_ROOT']) ) {
        ...
        $image_src = '/' . ltrim(preg_replace('/\\\\/', '/', $image_src), '/');
        $image_src = '/captcha/'.$captcha_config['code'];
    }
    $captcha_config['image_src'] = $image_src;
    ...
    if( isset($id) && !empty($id) ) {
        //session_start();
        //if(!isset($_SESSION['_CAPTCHA']['config'])) exit();
        ...
        $text_pos_x_min = 0;
        $text_pos_x_max = abs(($bg_width) - ($box_width));
        $text_pos_x = mt_rand($text_pos_x_min, $text_pos_x_max);
        $text_pos_y_min = $box_height;
        $text_pos_y_max = abs(($bg_height) - ($box_height / 2));
